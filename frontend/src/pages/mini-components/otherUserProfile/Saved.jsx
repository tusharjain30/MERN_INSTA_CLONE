...
const Saved = () => {
    ...
    const { otherUserProfile, user } = useSelector(state => state.user)

    return (
        <>
            <div className={`w-full ${otherUserProfile?.bookmarks?.length > 0 ? "grid grid-cols-3 gap-4" : ""} items-center justify-center p-8 mb-4`}>
                {
                  otherUserProfile?.bookmarks?.length > 0 ? (
                    otherUserProfile?.bookmarks?.map((curVal) => {
                            return (
